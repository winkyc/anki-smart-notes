...
class AdaptiveRateLimiter:
    ...
    """
    def __init__(self, initial_rpm: int = 5, min_rpm: int = 1, max_rpm: int = 60, manager=None, provider_name: str = ""):
    ...
    async def report_success(self):
        ...
        if self._rpm < self._max_rpm:
            ...
            if new_rpm > old_rpm:
                logger.debug(f"RateLimiter: Ramping up {self._provider_name} to {new_rpm} RPM")
                if self._manager:
    ...
    async def report_failure(self):
        ...
        if self._rpm > self._min_rpm:
            ...
            logger.warning(f"RateLimiter: 429 detected! Backing off {self._provider_name} to {new_rpm} RPM")
            if self._manager:
                self._manager.save_state()
    ...
    def get_limiter(self, provider: str) -> AdaptiveRateLimiter:
        if provider not in self.limiters:
            ...
            if provider in self.loaded_state:
                ...
                if 1 <= saved_rpm <= config["max"] * 1.5: # Allow slight overage if config changed
